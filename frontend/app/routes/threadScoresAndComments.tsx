...
export async function clientAction({ request, params }: Route.ClientActionArgs): Promise<ActionResponse> {
    ...
    for (const [key, value] of formData.entries()) {
        if (key.startsWith("score.")) {
            const scoreName = key.replace("score.", "");
    ...
    // Step 1: Create comment if provided
    if (comment) {
    ...
    // Step 2: Create scores if provided
        ...
        for (const [scoreName, scoreValue] of Object.entries(scores)) {
            const scoreResponse = await apiFetch('/api/scores', {
                ...
                body: {
                    ...
                    name: scoreName,
                    value: scoreValue,
                    commentId: commentId ?? null, // This will be undefined if no comment was created
                }
            });

            if (!scoreResponse.ok) {
                return { ok: false, error: scoreResponse.error };
            }
        }
    }

    return { ok: true, data: { commentId, scores: Object.keys(scores) } };
