...
app.post('/connect', async (c) => {
  ...
  const { messages, session, runConfig } = body;
  ...
  try {
    ...
    log.info(`[streaming] fetch error: ${fetchError}`);
  ...
  if (!response.body || !response.ok) {
    ...
    log.info(`[streaming] error response ${response.status}: ${errorText}`);
  ...
  processStream(runId, connection).catch((err) => {
...
async function callFastPatch(
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      log.error({ runId, status: resp.status, text }, '[streaming] fast-patch failed');
    }

    return { terminated: false };
  } catch (error) {
    log.error({ runId, err: error }, '[streaming] fast-patch call failed');
    return { terminated: false };
  }
}


async function processStream(runId: string, conn: LiveConnection) {
  const { reader, isChannelRun } = conn;
  ...
  try {
    ...
    for await (const data of parseAISDKStream(reader)) {
      ...
      switch (chunk.type) {
        ...
        case 'text-end': {
          ...
          const { terminated } = await callFastPatch(runId, conn, { type: 'item', content: { type: 'text', text } });
        ...
        case 'reasoning-end': {
          ...
          const { terminated } = await callFastPatch(runId, conn, { type: 'item', content: { type: 'reasoning', text } });
          if (terminated) throw new StreamTerminationError({ status: 'discarded', failReason: { message: 'Run terminated' } });
          break;
        }
        ...
        case 'tool-output-available': {
          ...
          if (state) {
            ...
            const { terminated } = await callFastPatch(runId, conn, {
              type: 'item',
    ...
    await callFastPatch(runId, conn, finalOp);
...
const port = Number(process.env.STREAMING_SERVER_PORT ?? '1991');
