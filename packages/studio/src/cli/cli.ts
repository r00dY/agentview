  ...
  program
    ...
    .action(async () => {
      ...
      printBanner([
        ...
        ["E-mail", `${slug}.${environment.handle}.[agent-name]@${getEmailRootDomain()}`],
...
const TUNNEL_STOP_TIMEOUT_MS = 3_000;

async function checkTunnelHealth(tunnelUrl: string): Promise<boolean> {
  // Hit our proxy's health endpoint through the tunnel. We match on the
  // body marker rather than headers because CF's edge stamps
  // `server: cloudflare` on every response — including healthy proxied
  // ones — so header signals aren't usable.
  try {
    const res = await fetch(new URL(HEALTH_PATH, tunnelUrl), {
      method: "GET",
    ...
    try {
      ...
      return parsed?.source === "agentview" && parsed?.ok === true;
    } catch {
      return false;
    }
  } catch {
...
async function stopTunnelWithTimeout(t: CloudflareTunnel): Promise<void> {
  // `untun`'s close can wedge after wake-from-sleep / network change. Bound
  // it so a stuck stop() can't block all future reconnect attempts.
  try {
    await Promise.race([
      t.stop(),
      new Promise<void>((_, reject) =>
        setTimeout(() => reject(new Error("tunnel-stop timeout")), TUNNEL_STOP_TIMEOUT_MS),
      ),
    ]);
  } catch { /* best-effort */ }
}
...
async function runProxyServer(client: StandardAgentViewClient) {
  ...
  const cleanup = async (exitCode: number) => {
    ...
    if (tunnel) {
      await stopTunnelWithTimeout(tunnel);
    }
  ...
  if (verbose) log(`Starting proxy server...`);
  proxy = await startProxyServer();
  if (verbose) log(`Proxy URL: ${ansi.dim}http://127.0.0.1:${proxy.port}${ansi.reset}`);

  if (verbose) log(`Opening Cloudflare tunnel...`);
  tunnel = await startCloudflareTunnel(proxy.port, { verbose });
  if (verbose) log(`Tunnel URL: ${ansi.dim}${tunnel.url}${ansi.reset}`);

  if (verbose) log(`Registering tunnel...`);
  ...
  async function regenerateIfUnhealthy() {
    ...
    tunnel = null;
    await stopTunnelWithTimeout(oldTunnel);
