...
async function checkTunnelHealth(tunnelUrl: string): Promise<boolean> {
  try {
    ...
    // Any response generated by Cloudflare's edge (rather than forwarded to
    // our origin) means the tunnel is down. CF tags its own pages with
    // `server: cloudflare`; our proxy doesn't set that header. This catches
    // the 4xx cases too — e.g. a reaped Quick Tunnel returning 404.
    if (res.headers.get("server")?.toLowerCase() === "cloudflare") return false;
